...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    k = p_cov_check.dot(h_jac.T).dot(
        np.linalg.inv(h_jac.dot(p_cov_check).dot(h_jac.T) + np.identity(3) * sensor_var)
    )

    # 3.2 Compute error state
    e = k.dot(y_k - p_check)
    # 3.3 Correct predicted state
    p_delta, v_delta, phi_delta = e[:3], e[3:6], e[6:]
    p_hat, v_hat = p_check + p_delta, v_check + v_delta
    q_hat = Quaternion(euler=phi_delta).quat_mult_right(q_check)
...
for k in range(
    ...
    rotation_matrix = Quaternion(
        q_est[k - 1][0], q_est[k - 1][1], q_est[k - 1][2], q_est[k - 1][3]
    ).to_mat()

    acc = rotation_matrix.dot(imu_f.data[k - 1] + g)
    p_est[k] = p_est[k - 1] + v_est[k - 1] * delta_t + ((delta_t**2) / 2) * acc
    v_est[k] = acc * delta_t
    q_est[k] = Quaternion(axis_angle=imu_w.data[k - 1] * delta_t).quat_mult_right(
        q_est[k - 1]
    )

    # 1.1 Linearize the motion model and compute Jacobians
    F, Q = np.identity(9), np.identity(6)
